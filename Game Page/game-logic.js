...
function checkAnswer(items, type, order, mode, answers) {
    ...
    const handleIncorrectAnswer = () => {
        updateGameStatus('Incorrect – Try again.', true);
        updateLivesAndCheckGameOver();
    };

    // Compare player's answers with the correct answer
    ...
    for (let i = 0; i < answers.length; i++) {
        if ((type === 'letters' && answers[i] !== correctAnswer[i]) ||
            (type === 'numbers' && answers[i] != correctAnswer[i].toString())) {
            handleIncorrectAnswer();
            return;
        }
    }
